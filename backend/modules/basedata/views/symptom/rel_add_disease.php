    ...
    <div class="d-list-form">
        ...
        <form class="disin fosub infc" method="post" name="search">
            ...
            <select class="level" name="class_level1" id="class_level1">
                <option>请选择</option>
                <?php
                foreach($allSymptom['relSymptom']['department'] as $k=>$v){
                ?>
...
<script>
rel_disease_department_level2_ajaxUrl = '<?php echo Url::to(['/basedata/symptom/ajax-department']);?>';
rel_disease_search_ajaxUrl = '<?php echo Url::to(['/basedata/symptom/ajax-add-relate-disease']);?>';
</script>
<script src="<?php echo Url::to('@domain/js/symptom/dialog_rel_disease.js')?>"></script>